...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    else if(opt==1)
    {
        ...
        for(int i=0;i<ARRAY_SIZE;i++)
        {
            //temp_sum = _mm256_set1_epi32(0);
            //temp_sum_cvt = _mm256_cvtepi32_ps(0.0);
			temp_sum_cvt = _mm256_set1_ps(0.0);
            for(int j=0;j<ARRAY_SIZE/2;j++)
            {
                one_vector = _mm256_set_epi32(A[i][j*2],A[i][j*2],A[i][j*2],A[i][j*2],  A[i][j*2+1],A[i][j*2+1],A[i][j*2+1],A[i][j*2+1]);
                one_vector_cvt = _mm256_cvtepi32_ps(one_vector);

                temp_row = _mm256_set_epi32(B[j*2][0],B[j*2][1],B[j*2][2],B[j*2][3],  B[j*2+1][0],B[j*2+1][1],B[j*2+1][2],B[j*2+1][3]);
                temp_row_cvt = _mm256_cvtepi32_ps(temp_row);
            ...
            //temp_sum = _mm256_cvtps_epi32(temp_sum_cvt);
